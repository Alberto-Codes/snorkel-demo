...
for _ in range(100):
    ...
    if np.random.rand() < 0.5:  # 50% chance to inject a specific word
        ...
        sentence[position] = np.random.choice(
            specific_words
        )  # replace a word at this position
...
transaction_amounts = np.random.choice(
    [45, 50, 23.45, 78.12, 56.78, 12.34, 89.56, 34.12], size=100
)

# Combine descriptions and amounts into a DataFrame
transactions = pd.DataFrame(
    {"Description": transaction_descriptions, "Amount": transaction_amounts}
)
